...
FUTURE = datetime(2999, 1, 1, tzinfo=ZoneInfo("Europe/Copenhagen"))
...
@pytest.mark.integration_test
async def test_future_org_unit_defers_the_event(
    create_org_unit,
    trigger_event,
    get_sent_mails,
) -> None:
    """An org unit that only exists in the future cannot be checked yet — it
    has no active state — so the event is deferred until the unit starts."""
    org_unit = await create_org_unit(name="Fremtiden", from_=FUTURE)

    r = await trigger_event("org_unit", org_unit)

    assert r.status_code == 425
    assert r.json()["detail"] == "Org unit starts in the future"
    not_before = datetime.fromisoformat(r.headers["X-Not-Before"])
    assert not_before == FUTURE
    assert await get_sent_mails() == []
