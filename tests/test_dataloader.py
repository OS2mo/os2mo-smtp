...
async def test_get_org_unit_address():
    org_unit_uuid = uuid4()
    test_data = OrgUnitAddressOrgUnits.parse_obj(
        {"objects": [{"current": {"addresses": [{"value": "test@test.dk"}]}}]}
    )

    mocked_mo_client = AsyncMock()

    mocked_mo_client.org_unit_address.return_value = test_data

    org_unit_address_response = await get_org_unit_address(
        mo=mocked_mo_client, uuid=org_unit_uuid
    )
    # expect a set of the email(s) in response
    expected_response = {"test@test.dk"}

    assert org_unit_address_response == expected_response
    mocked_mo_client.org_unit_address.assert_awaited_once_with(org_unit_uuid)
