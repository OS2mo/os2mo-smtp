...
def integration_test_environment_variables(
    ...
    # (e.g. a manager's address), producing extra, non-deterministic emails. A
    # test that needs a real listener re-enables exactly one via @pytest.mark.envvar.
    monkeypatch.setenv("ENABLE_ADDRESS_EVENTS", "False")
...
def get_sent_mails(
    ...
    Each message is enriched with its plain-text `body` — mailcatcher's
    message list omits the body and exposes it on a separate endpoint.
    """
    ...
    async def inner() -> list[dict]:
        ...
        for message in messages:
            body = await mailcatcher_client.get(f"/messages/{message['id']}.plain")
