...
async def get_ituser_validities(
    ...
    """Return all temporal validities for an ituser, oldest first.

    An empty list means the UUID doesn't resolve to an ituser at all.
    Termination is then detected by the caller as the absence of any
    open-ended (`to=None`) validity — a bounded update always leaves a
    trailing open-ended validity, so it doesn't get misread as terminated."""
